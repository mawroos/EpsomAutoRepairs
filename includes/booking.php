...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
    if( $_POST['booking_name'] != '' AND $_POST['booking_email'] != '' AND $_POST['booking_phone'] != '' AND $_POST['booking_date'] != '' ) {
        ...
        $name = htmlspecialchars($_POST['booking_name'], ENT_QUOTES, 'UTF-8');
        $email = filter_var($_POST['booking_email'], FILTER_SANITIZE_EMAIL);
        $phone = htmlspecialchars($_POST['booking_phone'], ENT_QUOTES, 'UTF-8');
        $date = htmlspecialchars($_POST['booking_date'], ENT_QUOTES, 'UTF-8');
        $post = htmlspecialchars($_POST['depertment_post'], ENT_QUOTES, 'UTF-8');

        $subject = 'New Message | Booking Form';
        ...
        if( !filter_var($email, FILTER_VALIDATE_EMAIL) ) {
            ...
            if( $spamCheck['passed'] ) {
                ...
                $date_line = !empty($date) ? "Booking date: $date<br><br>" : '';
                $post_line = !empty($post) ? "Department Post: $post<br><br>" : '';
                ...
                $body = "$name_line $email_line $phone_line $date_line $post_line $referrer";
