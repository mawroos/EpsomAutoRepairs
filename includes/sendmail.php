...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
    if( $_POST['form_name'] != '' AND $_POST['form_email'] != '' AND $_POST['form_subject'] != '' ) {
        ...
        $subject = htmlspecialchars($_POST['form_subject'], ENT_QUOTES, 'UTF-8');
        $phone = htmlspecialchars($_POST['form_phone'], ENT_QUOTES, 'UTF-8');
        $form_message = htmlspecialchars($_POST['form_message'], ENT_QUOTES, 'UTF-8');

        $subject = !empty($subject) ? $subject : 'New Message | Contact Form';
        ...
        if( !filter_var($email, FILTER_VALIDATE_EMAIL) ) {
            ...
            if( $spamCheck['passed'] ) {
                ...
                $subject_line = !empty($subject) ? "Subject: $subject<br><br>" : '';
                ...
                $body = "$name_line $email_line $subject_line $phone_line $message_line $referrer";
